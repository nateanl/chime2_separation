...
def prep_CHiME2_lists(base_dir, mask_type='ideal_amplitude'):
    ...
    if mask_type not in ['ideal_amplitude', 'phase_sensitive', 'ideal_complex']:
        # throw error if set_type is anything else
        raise Exception("mask type not in {'ideal_amplitude', 'phase_sensitive', 'ideal_complex'}")
        
    # build file lists
    train_list = [path+'/'+file for path,_,files in sorted(os.walk(base_dir)) for file in sorted(files) if (file.endswith('.mat') and mask_type in path)]
    return train_list